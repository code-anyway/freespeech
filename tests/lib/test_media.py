...
async def test_mix_spans(tmp_path):
    ...
    synth_file = await media.write_stream(
        stream=synth_stream, output_dir=tmp_path, extension="wav"
    )
...
async def test_keep_events(tmp_path):
    ...
    print(tmp_path)
    kept_file = await media.write_stream(
        stream=kept_stream,
        output_dir=tmp_path,
        extension="mp4",
    )
