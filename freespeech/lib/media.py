...
async def mix(
    ...
    return await write_stream(
        stream=mixed_audio, output_dir=output_dir, extension="wav"
    )
...
def trim_video_and_audio(file: str | PathLike, start_ms: int, end_ms: int):
    ...
    return video_trim  # ffmpeg.concat(video_trim, audio_trim, v=1, a=1)


async def write_stream(
    stream, output_dir: str | PathLike, extension: str, args={}
) -> Path:
    output_file = Path(f"{new_file(output_dir)}.{extension}")
    pipeline = ffmpeg.output(stream, **args, filename=output_file)
    await _run(pipeline)
...
def keep_events(summed_file: str | PathLike, spans: list[Tuple[str, int, int]]):
    ...
    return ffmpeg.concat(*bundle, v=1, a=1)
